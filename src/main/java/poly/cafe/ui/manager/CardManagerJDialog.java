...
List<Card> items = new ArrayList<>(); // tránh dùng List.of() vì nó immutable
...
public void fillToTable() {
    ...
    items = dao.findAll();
    items.forEach(item -> {
        // Kiểm tra index status có trong enum không, nếu không đặt về 0 (hoặc mặc định)
        int statusIndex = item.getStatus();
        Card.Status[] statuses = Card.Status.values();
        String statusName = (statusIndex >= 0 && statusIndex < statuses.length) 
...
public Card getForm() {
    ...
    try {
        ...
        entity.setId(null); // Hoặc xử lý theo logic bạn muốn
    }
...
public void create() {
    ...
    boolean exists = items.stream()
        .anyMatch(card -> card.getId() != null && card.getId().equals(entity.getId()));

    if (exists) {
        XDialog.alert("Thẻ với ID này đã tồn tại!");
        return;
    }
